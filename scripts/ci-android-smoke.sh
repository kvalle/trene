...
bundle_url='http://127.0.0.1:8081/index.ts.bundle?platform=android&dev=true&lazy=true&minify=false&app=no.kvalle.trene&modulesOnly=false&runModule=true&excludeSource=true&sourcePaths=url-server'
...
for _ in {1..60}; do
  if curl --fail --silent http://127.0.0.1:8081/status | grep -q 'packager-status:running'; then
    ...
    # Match React Native's app request so Maestro reuses this graph instead of
